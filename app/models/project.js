...
ProjectSchema.methods.checkRights = function (user, cb) {
    this.getOwner(_.bind(function (error, owner) {
        if (error) return cb('Server error')
        if (!owner) return cb('Project doesn\'t have owner yet')

        var noRights
        if (this.owner.user) {
            noRights = !user._id.equals(owner._id)
        } else {
            noRights = !_.find(owner.admins, function (admin) {
                return admin.equals(user._id)
            })
        }

        if (noRights) return cb('You don\'t have permissions to edit this project')
        cb(null, this)
    }, this))
}
